...
static char* config_get_value(const char* config_buffer, const char* value) {
	...
	if (!line)
		ERROR_EXIT("[Error]: Could not find config value: %s. "
...
void config_init(void) {
	...
	if (config_load() != 0)
		ERROR_EXIT("[Error]: Could not create or load config file.\n");
}

void config_key_bind(Input_Key key, const char* key_name) {
	SDL_Scancode scan_code = SDL_GetScancodeFromName(key_name);
	if (scan_code == SDL_SCANCODE_UNKNOWN)
		ERROR_RETURN(, "[Error]: Invalid scan code when binding key: %s\n", key_name);
