...
#define IO_READ_ERROR_MEMORY "[Error]: Not enough free memory to read file: %s\n"
...
File io_file_read(const char* path) {
	...
	while (true) {
		if (used + IO_READ_CHUNK_SIZE + 1 > size) {
			...
			if (size <= used) {
				free(data);
				ERROR_RETURN(file, "[Error]: Input file too large: %s\n", path);
			}
...
int io_file_write(void* buffer, size_t size, const char* path) {
	...
	if (!fp || ferror(fp))
		ERROR_RETURN(1, "[Error]: Cannot write file: %s.\n", path);
