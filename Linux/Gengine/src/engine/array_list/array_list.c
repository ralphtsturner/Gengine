...
Array_List* array_list_create(usize item_size, usize initial_capacity) {
	...
	if (!list)
		ERROR_RETURN(NULL, "[Error]: Could not allocate memory for Array_List\n");
...
usize array_list_append(Array_List* list, void* item) {
	if (list->len == list->capacity) {
		...
		if (!items)
			ERROR_RETURN(-1, "[Error]: Could not allocate memory for Array_List\n");
...
void* array_list_get(Array_List* list, usize index) {
	if (index >= list->len)
		ERROR_RETURN(NULL, "[Error]: Index out of bounds\n");
...
u8 array_list_remove(Array_List* list, usize index) {
	if (list->len == 0)
		ERROR_RETURN(1, "[Error]: List is empty\n");
	if (index >= list->len)
		ERROR_RETURN(1, "[Error]: Index out of bounds\n");
